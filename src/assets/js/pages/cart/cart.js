import "./cart.css";
import { resetMain, loadCartFromStorage, saveCartToStorage, calculateTotal } from "../../utils/helpers";
...
class ShoppingCartPage {
  ...
  renderItemList() {
    ...
    this.cart.forEach(item => {
      ...
      item.exclude ? itemElement.classList.add('item-cart-not-buy') : itemElement.classList.remove('item-cart-not-buy')
      this.cartList.appendChild(itemElement);
    });
    const orderSummaryCard = createOrderSummaryCard(this.cart);
    this.cartContainer = this.itemListContainer.querySelector('.cart-container');
    ...
    itemDiv.innerHTML = `
      ...
      <div class="item-details">
        ...
        <div class="item-controls">
          <label>
            Qty: <input type="number" value="${item.quantity}" min="1" class="quantity-input">
          </label>
          <button class="btn-remove">Remove from cart</button>
          <button class="btn-toggle">${item.excluded ? 'Include in order' : 'Exclude from order'}</button>
        </div>
  ...
  attachItemEventListeners(itemElement, item) {
    ...
    buttonToggle.addEventListener('click', () => {
      ...
      buttonToggle.textContent = obj.exclude ? 'Include in order' : 'Exclude from order'
      this.updateItemPrice(itemElement, item);
      saveCartToStorage(this.cart)
      this.renderItemList();
    });
